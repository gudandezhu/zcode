...
async function recoverStuckTasks() {
  ...
  for (const task of stuck) {
    ...
    if (latest) {
      await taskSvc.completeSession(latest.id, task.id, latest.status, latest.artifacts as unknown as Record<string, unknown>[]);

      // Handle stage advancement for completed sessions
      if (latest.status === "completed") {
        const hasAdvance = (latest.artifacts as unknown as Record<string, unknown>[]).some(
        ...
        if (hasAdvance) {
          ...
          if (nextStage) {
            ...
            await taskSvc.updateTask(task.id, { stage: nextStage as any });
            if (nextAgent) {
